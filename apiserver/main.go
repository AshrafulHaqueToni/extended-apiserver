...
import (
	...
	"crypto/x509"
	"flag"
	"github.com/AshrafulHaqueToni/extended-apiserver/lib/certstore"
	"github.com/AshrafulHaqueToni/extended-apiserver/lib/server"
...
func main() {
	...
	clientCert, clientKey, err := store.NewClientCertPair(cert.AltNames{
		DNSNames: []string{"Ashraful"},
	})
	if err != nil {
		log.Fatalln(err)
	}
	err = store.Write("Ashraful", clientCert, clientKey)
